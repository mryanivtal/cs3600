...
class LeakyReLU(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        dout_dx = torch.zeros_like(dout)
        dout_dx[x >= 0] = 1
        dout_dx[x < 0] = self.alpha
        dx = dout_dx * dout
        # ========================
...
class TanH(Layer):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        exp_x = torch.exp(x)
        exp_negx = torch.exp(-x)
        out = (exp_x - exp_negx) / (exp_x + exp_negx)
